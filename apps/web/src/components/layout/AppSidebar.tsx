...
export function AppSidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <aside className={`fixed left-0 top-0 h-screen bg-card border-r border-border flex flex-col z-40 transition-all duration-200 ${collapsed ? "w-[68px]" : "w-[220px]"}`}>
      ...
      <button onClick={onToggle}
        className="absolute -right-3 top-8 w-6 h-6 bg-card border border-border rounded-full flex items-center justify-center shadow-card hover:bg-secondary transition-colors">
